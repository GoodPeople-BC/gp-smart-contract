...
interface IGPVault {
    ...
    event DonationAborted(uint indexed donateId, address indexed recipient, address from, uint amount);
    ...
    function getDonationStatus(uint donateId) external view returns(DonationStatus status) ;
    ...
    function abortDonateProposal(uint donateId) external;
